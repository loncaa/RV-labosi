...
void SavePGM(char *Filename, unsigned char *I, int Width, int Height, BOOL bFlush)
{
	...
	/* sprema u matricu Height X Width int brojeve koji se nalaze na adresama pI.*/
	for(i = 0; i < Height; i++)
	{
		for(j = 0; j < Width; j++)
			fputc((int)(*(pI++)),fpbmp); 
...
/* koristi siftDemo4 program za pravljenje */
void CRV2DSIFT::CreateSIFT(IplImage* pInputImage)
{
	...
	SavePGM(SIFTInputImageFileName, pPixArray->pPix, pPixArray->Width, pPixArray->Height, TRUE);
